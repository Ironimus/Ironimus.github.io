...
export default class TypingAnimation extends Component {
	...
	async animateTyping(items, currentItem) {
		const { delayBefore = 1000, delayAfter = 2000, typeSpeed = 7, untypeSpeed = 20 } = this.props
		await this.typeText({
		...
		await this.animateTyping(items, currentItem === items.length - 1 ? 0 : currentItem + 1)
	}

	typeText = ({ text, delay, speed }) =>
		new Promise(resolve => {
	...
	render() {
		...
		return <WithBlinkingCursor stopped={isTyping}>{currentText}</WithBlinkingCursor>
	}
}
