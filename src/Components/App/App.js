...
export default class extends Component {
		...
		requestAnimationFrame(() =>
			...
			this.changeSection(
				this.state.sectionsRefs.findIndex(sectionRef => {
					...
					return offsetTop + offsetHeight > window.scrollY + window.innerHeight * 0.5
				})
			)
		)
	}
	...
	render() {
		...
		return (
			<Fragment>
				<EventListener target="window" onScroll={this.scrollHandler} />
				...
				<Menu activeSection={activeSection} onClick={this.scrollToSection}>
					{[
						'Home',
						'Skills',
						{
